...
class AStarPlanner:
    ...
    # 修改：增加了 show_process 参数
    def planning(self, sx, sy, gx, gy, show_process=False):
        """
        A* path search
        input:
        ...
        calc_count = 0 # 计数器，用于控制绘图频率

        while True:
            if not open_set:
                print("Open set is empty, finish searching.")
                break
            ...
            if show_process:
                ...
                if calc_count % 50 == 0:
                    plt.plot(self.calc_grid_position(current.x, self.min_x),
                             self.calc_grid_position(current.y, self.min_y), "xc", markersize=2)
                    plt.pause(0.001)
            # --------------------
